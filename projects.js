...
function openAddProjectModal() {
    ...
    try {
        ...
        console.warn("populateEngineerDatalists failed:", err);
    }
    try {
        setModalMode(false);
        resetModal();
        ...
        console.warn("openAddProjectModal failed:", err);
        modal.classList.add("open");
    }
}
...
document.addEventListener("click", (event) => {
    const trigger = event.target?.closest?.("#openAddProject");
    if (!trigger) return;
    openAddProjectModal();
});
...
saveBtn.addEventListener("click", () => {
    if (isEditMode && !isAdminUser) {
        alert("Only the admin can edit projects.");
        return;
    }

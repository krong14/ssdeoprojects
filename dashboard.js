...
function applyDocTooltipPlacement(card) {
  const tooltip = card?.querySelector(".doc-hover-tooltip");
  if (!tooltip) return;

  tooltip.classList.remove("flip-down");
  const rectUp = tooltip.getBoundingClientRect();

  // If the upward tooltip is clipped at the top, flip it below the card.
  if (rectUp.top < 8) {
...
function attachDocumentsCardHoverLists() {
  ...
  cards.forEach((card) => {
    ...
    card.addEventListener("mouseenter", () => applyDocTooltipPlacement(card));
    card.addEventListener("focusin", () => applyDocTooltipPlacement(card));
  });
}
...
function openDetailsModal(row) {
  ...
  const limits = row.dataset.limits || "";
  ...
  if (detailsLimits) detailsLimits.innerText = limits || "-";
